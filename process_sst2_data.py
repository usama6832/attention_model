...
def read_su_sentiment_rotten_tomatoes(dirname, lowercase=True):
   ...
   # more junk, and the replace necessary for sentence-phrase consistency
    ...
    phrase0 = [phrase for phrase in phrases if phrase.sentence_id is None and (phrase.sentiment > 0.6 or phrase.sentiment <= 0.4) ]
    sentences = [phrase for phrase in phrases if phrase.sentence_id is not None and (phrase.sentiment > 0.6 or phrase.sentiment <= 0.4) ]
...
#    sst_folder = sys.argv[2] # sst folder
    print ('load corpus')
    phrases, sentences, vocab = read_su_sentiment_rotten_tomatoes('./stanfordSentimentTreebank2')
...
#    cPickle.dump([ phrases, sentences, W, word_idx_map, vocab], open("sst2.p", "wb"))
